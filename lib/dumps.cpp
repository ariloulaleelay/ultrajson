...
PyObject* dumps(PyObject* self, PyObject *args, PyObject *kwargs) {
  ...
  if (!PyArg_ParseTuple(args, "O", &objectToDump))
    return NULL;

  encoderSingleton.reset();

  if (!TraverseObject(objectToDump, encoderSingleton)) {
    PyErr_Format(encoderSingleton.exceptionObject, encoderSingleton.error ? encoderSingleton.error : "unknown helljson error");
    return NULL;
  }
